#!/bin/bash

# parallel_trinotate_steps.bash - Split FASTA → PBS array jobs → merge outputs for Pfam/EggNOG
## NO QSUB for Master Script!
# Usage: bash parallel_trinotate_steps.bash pfam 141    # split+submit
#        bash parallel_trinotate_steps.bash pfam 141 m # merges the chunks, creates pfam.domtblout
#        bash parallel_trinotate_steps.bash eggnog 72 # split+submit
#        bash parallel_trinotate_steps.bash eggnog 72 m  # merges the chunks, creates eggnog.emapper.annotations

# =============================================================================
# parallel_trinotate_steps.bash - Parallel Trinotate Pfam/EggNOG Analysis
# =============================================================================
# AUTHOR: Iris Sammarco
#
# DATE: 25/03/2026
#
# PURPOSE: 
#   Splits large TransDecoder peptide FASTA files into chunks and submits PBS array jobs for parallel Pfam domain search (hmmscan) or EggNOG annotation.
#   Includes merge step to combine chunked outputs into final annotation files.
#
# FEATURES:
#   - Splits FASTA by sequence count (~10k seqs/chunk) using seqkit
#   - PBS array job submission (configurable chunk count)
#   - Automatic header-aware merging (Pfam: 22-line HMMER header, EggNOG: 5-line)
#   - Idempotent: skips if chunks exist or merge already done
#   - Comprehensive logging and error trapping
#
# REQUIREMENTS:
#   - PBS/MetaCentrum cluster environment
#   - trinotate_env (mamba/conda) with: seqkit, hmmer, eggnog-mapper
#   - Input: Trinity.fasta.transdecoder_dir/longest_orfs.pep (Pfam)
#   - Pfam-A.hmm in trinotate_data/
#
# USAGE:
#   1. SPLIT + SUBMIT (creates chunks, submits PBS array):
#      bash parallel_trinotate_steps.bash pfam 141      # 141 Pfam chunks
#      bash parallel_trinotate_steps.bash eggnog 141    # 141 EggNOG chunks
#
#   2. MERGE ONLY (after jobs complete):
#      bash parallel_trinotate_steps.bash pfam 141 m    # Merge Pfam → pfam.domtblout
#      bash parallel_trinotate_steps.bash eggnog 141 m  # Merge EggNOG → eggnog.emapper.annotations
#
# OUTPUTS:
#   - SPLIT: <split_dir>/chunk_<step>_001.pep/fasta, ...<N>.domtblout/emapper.annotations
#   - FINAL: trinotate_output/pfam.domtblout, eggnog.emapper.annotations
#   - LOGS:  logs/<step>_<jobid>_<arrayid>.out/err
#
# WORKFLOW:
#   1. split2 -s 10000 → ~10k sequences per chunk (adjustable)
#   2. PBS array: 1-N (N=chunks) → parallel hmmscan/emapper.py
#   3. merge: concatenate data (skip headers from chunk 2+)
#
# MONITORING:
#   qstat -u $USER | grep <step>
#   tail -f logs/<step>_<jobid>_<arrayid>.out
#
# =============================================================================

set -euo pipefail
trap 'echo "[ERROR] Line $LINENO: ${BASH_COMMAND} failed on $(date)" >&2; exit 1' ERR

if [[ $# -lt 2 || $# -gt 3 ]]; then
    echo "Usage: $0 {pfam|eggnog} N_CHUNKS [m]" >&2
    echo "  N_CHUNKS: number of PBS array jobs (e.g. 20)" >&2
    echo "  m: merge-only mode (optional)" >&2
    exit 1
fi

# Environment & Paths
export TMPDIR=$SCRATCHDIR # This will force the application to place the temporary files into scratch directory instead of a /tmp directory
module add mambaforge
export CONDA_ENVS_PATH=/storage/pruhonice1-ibot/home/irissammarco/.conda/envs
mamba activate trinotate_env

STEP=$1
N_CHUNKS=$2
MERGE_ONLY=${3:-}
OUTDIR="/storage/plzen1/home/irissammarco/Festuca_RNA_assembly/assembly/trinotate_output"
PEP_LONG="${OUTDIR}/Trinity.fasta.transdecoder_dir/longest_orfs.pep"
PFAM_HMM="/storage/plzen1/home/irissammarco/Festuca_RNA_assembly/assembly/trinotate_data/Pfam-A.hmm"

cd "${OUTDIR}"

case "${STEP}" in
    pfam)
        OUTPUT="pfam.domtblout"
        SPLIT_DIR="${OUTDIR}/Trinity.fasta.transdecoder_dir/longest_orfs.pep.split"
        PBS_SCRIPT="trinotate_03a_pfam_array.pbs"
        INPUT_FILE="${PEP_LONG}"
		CHUNK_EXT="pep"
        ;;
    eggnog)
        OUTPUT="eggnog.emapper.annotations"
        SPLIT_DIR="${OUTDIR}/Trinity.fasta.transdecoder.pep.split"
        PBS_SCRIPT="trinotate_05a_eggnog_array.pbs"
        INPUT_FILE="${OUTDIR}/Trinity.fasta.transdecoder.pep"
		CHUNK_EXT="pep"
        ;;
    *)
        echo "ERROR: Step must be 'pfam' or 'eggnog'" >&2
        exit 1
        ;;
esac

[[ -s "${INPUT_FILE}" ]] || { echo "[FATAL] Missing input: ${INPUT_FILE}" >&2; exit 1; }

if [[ -z "${MERGE_ONLY}" ]]; then
    echo "[INFO] === SPLIT + SUBMIT MODE: ${STEP^^} (${N_CHUNKS} chunks) ==="
    
    # 1. Use existing chunks OR create new ones
	if [[ ! -d "${SPLIT_DIR}" || ! -f "${SPLIT_DIR}/chunk_${STEP}_001.${CHUNK_EXT}" ]]; then
        echo "[INFO] Creating new chunks in ${SPLIT_DIR}"
        rm -rf "${SPLIT_DIR}"
        mkdir -p "${SPLIT_DIR}"
        seqkit split2 -s 10000 "${INPUT_FILE}" -o "${SPLIT_DIR}" --by-size-prefix "chunk_${STEP}_"
    fi
	
	# 2. Count & verify chunks 
    CHUNKS=$(ls "${SPLIT_DIR}"/chunk_${STEP}_*.${CHUNK_EXT} 2>/dev/null | wc -l || echo 0)
    echo "[INFO] Found ${CHUNKS} chunks in ${SPLIT_DIR}"
    [[ ${CHUNKS} -ge 1 ]] || { echo "[FATAL] No chunks found!" >&2; exit 1; }

	# 3. Submit PBS array
	mkdir -p logs
	qsub -J 1-${CHUNKS}:1 \
		-N "${STEP}_parallel_${CHUNKS}" \
		-o "logs/${STEP}_%A_%a.out" \
		-e "logs/${STEP}_%A_%a.err" \
		"${PBS_SCRIPT}"
	echo "[INFO] Submitted ${CHUNKS} ${STEP^^} jobs. Monitor: qstat -u \$USER | grep ${STEP}"
	   
elif [[ "${MERGE_ONLY}" == "m" ]]; then
    echo "[INFO] === MERGE MODE: ${STEP^^} ==="
    
    if [[ -s "${OUTPUT}" && -s ".${STEP}.merged.done" ]]; then
        echo "[INFO] ${OUTPUT} already merged, skipping"
        exit 0
    fi
    
    case "${STEP}" in
        pfam)
            # Merge domtblout (HMMER3: 22-line header from first file only)
            rm -f "${OUTPUT}"
            FIRST=1
            for f in "${SPLIT_DIR}"/chunk_${STEP}_*.domtblout; do
                [[ -s "$f" ]] || continue
                if [[ $FIRST -eq 1 ]]; then
                    cat "$f" >> "${OUTPUT}"  # Header + data
                    FIRST=0
                else
                    tail -n +23 "$f" >> "${OUTPUT}"  # Skip 22-line header
                fi
            done
            [[ -s "${OUTPUT}" ]] && touch ".${STEP}.merged.done"
            echo "[INFO] Merged ${OUTPUT} ($(wc -l < "${OUTPUT}") lines)"
            ;;
        eggnog)
            # Merge eggnog (5-line header from first, data from all)
            rm -f "${OUTPUT}"
            FIRST=1
            for f in "${SPLIT_DIR}"/chunk_${STEP}_*.emapper.annotations; do
                [[ -s "$f" ]] || continue
                if [[ $FIRST -eq 1 ]]; then
                    head -5 "$f" > "${OUTPUT}"  # 5-line header
                    tail -n +6 "$f" >> "${OUTPUT}"  # Data only
                    FIRST=0
                else
                    tail -n +6 "$f" >> "${OUTPUT}"  # Data only
                fi
            done
            [[ -s "${OUTPUT}" ]] && touch ".${STEP}.merged.done"
            echo "[INFO] Merged ${OUTPUT} ($(wc -l < "${OUTPUT}") lines)"
            ;;
    esac
fi